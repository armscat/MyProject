...
import com.armscat.photoeditors.adapter.ImageGridAdapter;
import com.armscat.photoeditors.bean.ImageFolder;
import com.armscat.photoeditors.bean.ImageItem;
import com.armscat.photoeditors.util.SystemBarTintManager;
import com.armscat.photoeditors.util.Utils;
import com.armscat.photoeditors.view.ImageDataSource;
...
public class ImageGridActivity extends TranslucentActivity implements ImageDataSource.OnImagesLoadedListener, ImageGridAdapter.OnImageItemClickListener, ImagePicker.OnImageSelectedListener, View.OnClickListener {
    ...
    public static final int PERMISSIONS_REQUEST_CAMERA = 123;
...
//        int size = getIntent().getIntExtra(ImagePicker.MAX_PHOTO_NUMBER, 0) > 16 ? 16 : getIntent().getIntExtra(ImagePicker.MAX_PHOTO_NUMBER, 0);
//        int size = ImagePicker.getInstance().getSelectLimit();
...
//        imagePicker.setSelectLimit(size >= 0 ? size : 0);
        ...
        if (data != null && data.getExtras() != null) {
            ...
            if (directPhoto) {
                if (!(checkPermission(Manifest.permission.CAMERA))) {
                    ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.CAMERA}, ImageGridActivity.PERMISSIONS_REQUEST_CAMERA);
                } else {
        ...
        mBtnDir = (Button) findViewById(R.id.btn_all);
...
//                isOrigin = data.getBooleanExtra(ImagePreviewActivity.ISORIGIN, false);
            ...
            if (resultCode == RESULT_OK && requestCode == ImagePicker.REQUEST_CODE_TAKE) {
                ...
                pthotoPath = getSDPath()+ "/" + UUID.randomUUID().toString() + ".png";
...
//                    startActivityForResult(intent, ImagePicker.REQUEST_CODE_CROP);  //单选需要裁剪，进入裁剪界面
                    ...
                    Intent intent = new Intent(ImageGridActivity.this, IMGEditActivity.class);
            ...
            }else if (resultCode == RESULT_OK && requestCode == ImagePicker.REQUEST_CODE_CROP) {
                //发送广播通知图片增加了
                ImagePicker.galleryAddPic(this, imagePicker.getTakeImageFile());


                ImageItem imageItem = new ImageItem();
                imageItem.imagePath = pthotoPath;
                imagePicker.clearSelectedImages();
                imagePicker.addSelectedImageItem(0, imageItem, true);
